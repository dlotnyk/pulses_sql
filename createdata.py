...
class sql_create():
    ...
    tables = {'0bar': ('hec_0bar', 'ic_0bar'), '9psi': (
        'hec_9psi', 'ic_9psi'), '22bar': ('hec_22bar', 'ic_22bar')}
...
#        ID int NOT NULL UNIQUE,
        assert type(tb_name1) is str and type(
            tb_name2) is str, "tables name should be str"
...
#        print(table_IC)
    ...
    def drop_f(self, tb_name):
        ...
        assert type(list(tb_name.values())[
                    0]) is dict, "input parameter should be a nested dict"
    ...
    def __insert_sql(self, tab_name1, tab_name2, val1, val2):
        ...
        assert type(tab_name1) is str and type(
            tab_name2) is str, "Table name should be str"
        assert type(val1) is tuple and type(
            val2) is tuple, "values should be combined into tuples"
    ...
    def insert_tables(self, d_value):
        ...
        assert type(list(d_value.values())[
                    2]) is tuple, "input parameter should obtain a tuple"
        path1 = d_value['path1']
        path2 = d_value['path2']
        # truncate
        trun1 = ("ALTER TABLE {} {} = 1".format(
            d_value['tables'][0], self.autoinc))
        trun2 = ("ALTER TABLE {} {} = 1".format(
            d_value['tables'][1], self.autoinc))
        ...
        for p in path1:
            data = np.genfromtxt(
                p, unpack=True, skip_header=1, usecols=(2, 6, 7, 13))
            if counter == 0:
                data1 = data.copy()
                counter += 1
            else:
        ...
        for p1 in path2:
            data21 = np.genfromtxt(
                p1, unpack=True, skip_header=1, usecols=(2, 6, 7, 13))
            if counter == 0:
                data2 = data21.copy()
                counter += 1
            else:
...
#                    val22.append(str(jj))
            ...
            self.__insert_sql(d_value['tables'][0],
                              d_value['tables'][1], val1, val2)
            counter += 1
        self.cnx.commit()
#        print(counter)
        tab1_count = ("SELECT COUNT(`index`) FROM `{}`".format(
            d_value['tables'][0]))
        tab2_count = ("SELECT COUNT(`index`) FROM `{}`".format(
            d_value['tables'][1]))
    ...
    def select_col(self, r_list, tb_name):
        assert type(r_list) is list and type(
            tb_name) is str, "list of rows should have a type list"
        cur = ''
        for ii in r_list:
            cur += "`"+ii+"`, "
        cur = cur[0:-2]
        sel = ("SELECT "+cur +
               "FROM `{0}` WHERE 1 ORDER BY `{0}`.`index` ASC".format(tb_name))
