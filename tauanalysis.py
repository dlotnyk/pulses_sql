...
#---------------------------------------------------------
    ...
    def __repr__(self):
        return "Pulse analysis class"
    
    def __str__(self):
#        s=self.sett['pressure']
        return "Pulse analysis class"
#---------------------------------------------------------      
...
#---------------------------------------------------------
    def my_logger(orig_func):
        ...
        import logging
        import datetime
        logging.basicConfig(filename='work1.log'.format(orig_func.__name__), level=logging.ERROR)
...
#                print(pul_id, self.pulse_id[pul_id])
                ...
                self.__set_pulse(count+1,self.mval+1000*nump)
                nump+=1
            else:
                self.rawdata1[4][count]=ini
                self.__set_pulse(count+1,ini)
            count+=1
            ini+=increm
        self.cnx.commit()
 #---------------------------------------------------------                   
...
#        UPDATE `table_one` SET `value` = '200' WHERE `table_one`.`id` = 1;
        set_v=("UPDATE `{0}` SET `pulse` = '{1}' WHERE `{0}`.`index` = {2}".format(self.tables[self.sett['pressure']][0],val,ind))
